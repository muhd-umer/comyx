...
from simcomm.core import get_rvs
from simcomm.core.propagation import get_noise_power, get_pathloss
from simcomm.utils import db2pow, dbm2pow, get_distance, pow2db, qfunc
...
def main(N, link_option, custom_run, save_path):
    ...
    # Channel generation
    print(f"{Fore.GREEN}Computing ...{Style.RESET_ALL}")
    ...
    simulation_V_ic_noK = zeta_ic * rho * h_ic**2
    simulation_V_ic_u2 = zeta_ic * rho * H_ic_u2**2
    simulation_V_ic_u2_noK = zeta_ic * rho * h_ic_u2**2
    ...
    simulation_B_ic_noK = (rho * h_ic_prime**2) + 1
    simulation_B_ic_u2_noK = (rho * h_ic_prime_u2**2) + 1
    ...
    simulation_SINR_ic_noK = simulation_V_ic_noK / simulation_B_ic_noK
    simulation_SINR_ic_u2_noK = simulation_V_ic_u2_noK / simulation_B_ic_u2_noK
    ...
    simulation_V_f_noK = zeta_if * rho * h_if**2 + zeta_if_hat * rho * h_if_hat**2
    ...
    simulation_W_f_noK = zeta_ic * rho * h_if**2 + zeta_ic_hat * rho * h_if_hat**2
    simulation_B_f = simulation_W_f + 1
    simulation_B_f_noK = simulation_W_f_noK + 1
    ...
    simulation_SINR_f_noK = simulation_V_f_noK / simulation_B_f_noK

    simulation_SINR_f_nonC_B1 = (rho * h_if**2) / (1 + rho * h_if_hat**2)
    simulation_SINR_f_nonC_B2 = (rho * h_if_hat**2) / (1 + rho * h_if**2)
    simulation_SINR_f_nonC = (
        zeta_if * simulation_SINR_f_nonC_B1 + zeta_if_hat * simulation_SINR_f_nonC_B2
    ) / (
        zeta_ic * simulation_SINR_f_nonC_B2
        + zeta_ic_hat * simulation_SINR_f_nonC_B1
        + 1
    )
    ...
    simulation_rate_ic = np.log2(1 + np.mean(simulation_SINR_ic, axis=0)) - scale_ic
    simulation_rate_ic_u2 = (
        np.log2(1 + np.mean(simulation_SINR_ic_u2, axis=0)) - scale_ic
    )
    simulation_rate_f = np.log2(1 + np.mean(simulation_SINR_f, axis=0)) - scale_f
    ...
    simulation_outage_ic = qfunc(
        (np.mean(pow2db(simulation_SINR_ic) - (-N0) - (-110), axis=0) / SIGMA)
    )
    simulation_outage_ic_u2 = qfunc(
        (np.mean(pow2db(simulation_SINR_ic_u2) - (-N0) - (-110), axis=0) / SIGMA)
    )
    outage_ic_u2 = qfunc(
        (
            np.mean(
                pow2db(
                    sp.stats.betaprime.rvs(
                        ...
                        k_B_ic_u2,
                        loc=0,
                        scale=theta_V_ic_u2 / theta_B_ic_u2,
                        size=(N * 20, len(rho)),
                    )
                )
                - (-N0)
    ...
    simulation_outage_f = qfunc(
        (np.mean(pow2db(simulation_SINR_f) - (-N0) - (-110), axis=0) / SIGMA)
    )
    ...
    if save_path != "":
        ...
        if not ris_enhanced and not comp_enabled:
            simulation_outage_f_nonC = qfunc(
                (
                    np.mean(pow2db(simulation_SINR_f_nonC) - (-N0) - (-110), axis=0)
                    / SIGMA
                )
            )
            outage_f_nonC = qfunc(
                (
                    np.mean(
                        pow2db(simulation_SINR_f_nonC) - scale_f - (-N0) - (-110),
                        axis=0,
                    )
                    / SIGMA
                )
            )
            simulation_rate_f_nonC = (
                np.log2(1 + np.mean(simulation_SINR_f_nonC, axis=0)) - scale_f
            )
            rate_f_nonC = np.log2(1 + np.mean(simulation_SINR_f_nonC, axis=0))
            ...
            io.savemat(
                res_file,
                {
                    ...
                    "outage": [
                        outage_f_nonC,
                        simulation_outage_f_nonC,
                    ],
                },
            )
        elif not ris_enhanced:
            simulation_outage_ic_noK = qfunc(
                (
                    np.mean(pow2db(simulation_SINR_ic_noK) - (-N0) - (-110), axis=0)
                    / SIGMA
                )
            )
            outage_ic_noK = qfunc(
                (
                    np.mean(
                        pow2db(simulation_SINR_ic_noK) - scale_f - (-N0) - (-110),
                        axis=0,
                    )
                    / SIGMA
                )
            )
            simulation_outage_ic_u2_noK = qfunc(
                (
                    np.mean(pow2db(simulation_SINR_ic_u2_noK) - (-N0) - (-110), axis=0)
                    / SIGMA
                )
            )
            outage_ic_u2_noK = qfunc(
                (
                    np.mean(
                        pow2db(simulation_SINR_ic_u2_noK) - scale_f - (-N0) - (-110),
                        axis=0,
                    )
                    / SIGMA
                )
            ...
            simulation_outage_f_noK = qfunc(
                (
                    np.mean(pow2db(simulation_SINR_f_noK) - (-N0) - (-110), axis=0)
                    / SIGMA
                )
            )
            outage_f_noK = qfunc(
                (
                    np.mean(
                        pow2db(simulation_SINR_f_noK) - scale_f - (-N0) - (-110), axis=0
                    )
                    / SIGMA
                )
            )
            simulation_rate_ic_noK = (
                np.log2(1 + np.mean(simulation_SINR_ic_noK, axis=0)) - scale_f
            )
            simulation_rate_ic_u2_noK = (
                np.log2(1 + np.mean(simulation_SINR_ic_u2_noK, axis=0)) - scale_f
            )
            simulation_rate_f_noK = (
                np.log2(1 + np.mean(simulation_SINR_f_noK, axis=0)) - scale_f
            )
            rate_ic_noK = np.log2(1 + np.mean(simulation_SINR_ic_noK, axis=0))
            rate_ic_u2_noK = np.log2(1 + np.mean(simulation_SINR_ic_u2_noK, axis=0))
            rate_f_noK = np.log2(1 + np.mean(simulation_SINR_f_noK, axis=0))
            ...
            io.savemat(
                res_file,
                {
                    ...
                    "outage": [
                        ...
                        outage_ic_u2_noK,
                        outage_f_noK,
                        simulation_outage_ic_noK,
                        simulation_outage_ic_u2_noK,
                        simulation_outage_f_noK,
                    ],
                },
            )
        else:
